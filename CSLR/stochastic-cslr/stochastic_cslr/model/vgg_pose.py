...
from temporal_model import Conv1D, EncoderRNN
...
class VGGPose_Conv1D(nn.Module):
    ...
    def forward(self, x, pose_given=None, teacher_ratio=0.0):
        B, C, T, H, W = x.shape
...
class VGGPose_Conv1D_RNN(nn.Module):
    def __init__(self, num_classes, rnn_type='LSTM', input_channels=3):
    ...
    def forward(self, x, len_x, pose_given=None, teacher_ratio=0):
        B, C, T, H, W = x.shape
        if pose_given is not None:
            pose_given = pose_given.reshape(B*T, 7, 2)
        # (B, C, T, H, W) 
